 ...
 * @author *-xguo0<@
 */
public final class MinkeTable implements StorageTable, Recycable {
    ...
    public long get(long pKey, long options, GetInfo info) {
        ...
        long pResult = page.get(pKey, info);
        return pResult;
    }
    ...
    public long get(long pKey, boolean track) {
        ...
        return page.get(pKey, track, null);
    }
    ...
    public long get(KeyBytes key) {
        ...
        return page.get(key.getAddress(), null);
    }
    
    @Override
    public boolean exist(long pKey) {
        return get(pKey, 0, null) > 1;
    }
    
    @Override
    public long getIndex(long pKey, long options, GetInfo info) {
        long pIndexRow = get(pKey, 0, info);
        if (pIndexRow <= 1) return pIndexRow;
        IndexRow row = new IndexRow(pIndexRow);
        return row.getRowKeyAddress();
    }
            ...
            while (scanner.next()) {
                ...
                if (this.type == TableType.DATA) {
                    ...
                    if (pRowKey != 0) {
                        ...
                        newpage.putIndex(version, pKey, pRowKey, misc);
                    }
                }
            }
    ...
    private long split1(MinkePage page, long pIncomingKey, int incomingSize, long version, long pRowKey, byte misc) {
        return split1(page, pIncomingKey, incomingSize, (MergeStream ms)-> {
            ms.setMergeIndexData(version, pIncomingKey, pRowKey, misc);
        });
    }
    ...
    long split1(MinkePage page, long pIncomingKey, int incomingSize, Consumer<MergeStream> cb) {
        ...
        try {
            ...
            /* copy on write is disabled due to suspicion that it will cause dead loop. no proof yet 
    ...
    public void putIndex(long pIndexKey, long pRowKey, byte misc) {
        putIndex_(0, pIndexKey, pRowKey, misc);
    }
    
    @Override
    public void putIndex(long version, long pIndexKey, long pRowKey, byte misc) {
        putIndex_(version, pIndexKey, pRowKey, misc);
    }
    
    synchronized long putIndex_(long version, long pIndexKey, long pRowKey, byte misc) {
        for (;;) {
            ...
            try {
                return page.putIndex(version, pIndexKey, pRowKey, misc);
            }
            catch (OutOfPageRange x) {
                grow(pIndexKey);
            }
                ...
                int size = KeyBytes.getRawSize(pIndexKey) + KeyBytes.getRawSize(pRowKey) + 1;
                 return split1(page, pIndexKey, size, version, pRowKey, misc);
            }
        }
    }
    ...
    /**
     * compact pages in the table when the wasted bytes is higher than the threshold
     * 
     * @param threshold
     * @throws IOException 
     */
    public void compact(int threshold) throws IOException {
        for (MinkePage i = this.pages.getCeilingPage(KeyBytes.getMinKey()); i!= null;) {
            synchronized(this) {
                MinkePage next = this.pages.getHigherPage(i.getStartKeyPointer());
                if (next == null) {
                    break;
                }
                int totalUnusedBytes = getUnusedBytes(i) + getUnusedBytes(next);
                if ((totalUnusedBytes - this.minke.pageSize) >= threshold) {
                    i = merge(i, next);
                }
                else {
                    i = next;
    ...
    private void copy(MinkePage source, MinkePage target) {
        MinkePage.Scanner scanner = source.scanAll(ScanOptions.SHOW_DELETE_MARK);
        if (scanner != null) {
            while (scanner.next()) {
                target.put(getType(), scanner);
            }
        }
        RangeScanner ranges = source.getAllRanges();
        for (Range i=ranges.next(); i!=null; i=ranges.next()) {
            target.putRange(i);
        }
    }
    
    private MinkePage merge(MinkePage first, MinkePage next) throws IOException {
        _log.debug("merging: {} {} tableId={}", hex(first.id), hex(next.id), this.tableId);
        MinkePage result = this.minke.alloc(this.tableId, first.getStartKey(), next.getEndKey());
        copy(first, result);
        copy(next, result);
        this.pages.put(result);
        this.pages.remove(next);
        this.minke.freePage(first);
        this.minke.freePage(next);
        return result;
    }

    private int getUnusedBytes(MinkePage page) {
        return this.minke.pageSize - page.getUsage() + page.getWastedBytes();
    }
    
    public void shrink(int threshold) throws IOException {
        for (MinkePage page:this.pages.values()) {
            // prevent concurrency
            synchronized (this) {
                // leave active pages alone. 
                if (page.getState() == PageState.CARBONFREEZED) {
                    if (page.getWastedBytes() >= threshold) {
                        compact(page);
                    }
                }
            }
     ...
     * @throws IOException 
     */
    public synchronized void compact(MinkePage page) throws IOException {
        this.mergeSplit1(page, 1, null);
    }
