...
public class SetOptionPacket extends RecievePacket {
    public byte clientMultiStmts;
    
    public SetOptionPacket(int command) {
        super(command);
    }

    @Override
    public void read(MysqlServerHandler handler, ByteBuf in) {
    	clientMultiStmts = BufferUtils.readByte(in);
    }
