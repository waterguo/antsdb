...
public class MysqlSlave implements BinlogEventListener {
    ...
    private int buildInsertPstmt(TableMeta meta) throws SQLException{
        ...
        PreparedStatement script = prepare(session, sqlStr);
    ...
    private void executeInsertPstmt(int stmtId, List<Row> rows) {
        ...
        for (Row row: rows) {
            ...
            try {
                Object result = session.run(script, param, null);
                checkResult(result);
            }
    ...
    private PreparedStatement prepare(Session session, String sql) throws SQLException {
        CharBuffer cbuf = CharBuffer.wrap(sql);
        return session.prepare(cbuf);
    }
