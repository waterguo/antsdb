 ...
 * @author *-xguo0<@
 */
public class FuncParseMinuteSecond extends Function {
    private static Pattern _ptn = Pattern.compile("\\s*(\\d+):(\\d+)\\s*");
    
    private Operator upstream;

    public FuncParseMinuteSecond(Operator upstream) {
    ...
    public long eval(VdmContext ctx, Heap heap, Parameters params, long pRecord) {
        ...
        int result = Integer.parseInt(m.group(1)) * 60 + Integer.parseInt(m.group(2));
