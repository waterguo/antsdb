...
public class IndexEntryHandler {
    ...
    void insert(Heap heap, 
                ...
                Row oldRow) {
        for (;;) {
            long error = this.gtable.insertIndex(hsession, trx.getTrxId(), pIndexKey, pRowKey, misc, timeout);
            ...
            if (HumpbackError.isSuccess(error)) {
                ...
                if (error == HumpbackError.MISSING) {
                    ...
                    msg = HumpbackError.toString(error);
                }
    ...
    void delete(Heap heap, HumpbackSession hsession, Transaction trx, long pIndexKey, Row row, int timeout) {
        long error = this.gtable.delete(hsession, trx.getTrxId(), pIndexKey, timeout);
        if (!HumpbackError.isSuccess(error)) {
            String msg;
            if (error == HumpbackError.MISSING) {
                msg = "index entry not found";
