...
public class AutoCaster {
    static Pattern _ptnYear = Pattern.compile("(\\d+)");
    ...
    public static long toUnicode(Heap heap, long pValue) {
        long pResult = toString(heap, pValue);
        if (pResult == 0) return 0;
        byte format = Value.getFormat(null, pResult);
        if (format == Value.FORMAT_UNICODE16) return pResult;
        pResult = new FishUtf8(pResult).toUnicode16(heap);
        return pResult;
    }
    ...
    public static long toString(Heap heap, long pValue) {
        ...
        case Value.FORMAT_BIGINT: {
            BigInteger value = BigInt.get(heap, pValue);
            return Unicode16.allocSet(heap, String.valueOf(value));
        }
        case Value.FORMAT_DATE: {
        ...
        case Value.FORMAT_CLOB_REF: {
            return Unicode16.allocSet(heap, "CLOB");
        }
        case Value.FORMAT_BLOB_REF: {
            return Unicode16.allocSet(heap, "BLOB");
        }
        default:
    ...
    public static Long getLong(long pValue) {
        ...
        case Value.FORMAT_BIGINT:
            return BigInt.get(null, pValue).longValue();
        case Value.FORMAT_DECIMAL:
    ...
    private static Date parseDate(String text) {
        ...
        if (m.find()) {
            ...
            Matcher y = _ptnYear.matcher(text);
            if (y.find()) {
    ...
    public static double getDouble(long pValue) {
        ...
        case Value.FORMAT_FLOAT4:
            return (double)Float4.get(null, pValue);
    ...
    public static BigDecimal getDecimal(long pValue) {
        ...
        case Value.FORMAT_INT8:
            ...
            break;
        case Value.FORMAT_BIGINT:
            result = new BigDecimal(BigInt.get(null, pValue));
            break;
        case Value.FORMAT_FAST_DECIMAL:
        ...
        case Value.FORMAT_UNICODE16:
            ...
            break;
        case Value.FORMAT_BOOL:
            result = FishBool.get(null, pValue) ? BigDecimal.ONE : BigDecimal.ZERO;
            break;
        default:
            throw new IllegalArgumentException(String.valueOf(format));
        }
