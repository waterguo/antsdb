 ...
 * @author *-xguo0<@
 */
public class FuncParseDaySecond extends Function {
    private static Pattern _ptn = Pattern.compile("\\s*(\\d+)\\s+(\\d+):(\\d+):(\\d+)\\s*");
    
    private Operator upstream;

    public FuncParseDaySecond(Operator upstream) {
    ...
    public long eval(VdmContext ctx, Heap heap, Parameters params, long pRecord) {
        ...
        int result = day * 24 * 60 * 60 + hour * 60 * 60 + minute * 60 + second;
