...
public class HSelectIndex extends CursorMaker {
    ...
    private class MyCursor extends CursorWithHeap {
        ...
        public long next() {
            ...
            if (scan.next()) {
                ...
                Record.set(pRecord, 1, Unicode16.allocSet(getHeap(), scan.getLocation()));
                Record.set(pRecord, 2, AutoCaster.toString(getHeap(), pRowKey));
                Record.set(pRecord, 3, this.scan.getMisc());
                return pRecord;
            }
            else {
                return 0;
    ...
    public HSelectIndex(GTable gindex) {
        ...
        this.meta.addColumn(new FieldMeta("01", DataType.varchar()));
    ...
    public Object run(VdmContext ctx, Parameters params, long pMaster) {
        ...
        RowIterator scan = gindex.scan(trx.getTrxId(), trx.getTrxTs(), 0, 0, NO_CACHE);
