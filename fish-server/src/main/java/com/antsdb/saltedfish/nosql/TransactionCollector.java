...
class TransactionCollector implements Runnable {
	...
	static long renderOldest(Humpback humpback) {
		...
		long hbaseStartTrxId = Long.MIN_VALUE;
		HBaseStorageService hbase = humpback.getHBaseService();
		if (hbase != null) {
			hbaseStartTrxId = hbase.getStartTrxId();
			if (hbaseStartTrxId == 0) {
				return 0;
			}
		}
		...
		if ((oldestTrxId <= activeSessionStartTrxId) || (oldestTrxId <= hbaseStartTrxId)) {
			return 0;
		}
		...
		_log.trace(
			...
			oldestTrxId,
			secondOldestTrxId);
		long startTrxId = max(activeSessionStartTrxId, hbaseStartTrxId, currentStartTrxId, secondOldestTrxId);
	...
	private static long max(long n1, long n2, long n3, long n4) {
